...
There are many other courses [AN: "books"? and if you make this change, perhaps change the link] that are also very informative [reference books](https://www.quora.com/Which-are-the-best-Unix-Linux-reference-books).
...
knitr::include_graphics("img/unix/filesystem.png") 
```
...
```{r, echo=FALSE,  out.width = "500px", out.extra='style="display: block; margin-left: auto; margin-right: auto; background-color: #000; padding:3px;"'}
knitr::include_graphics("img/unix/windows-filesystem-from-root.png") 
```

while on the Mac, it will be like this:

```{r, echo=FALSE,  out.width = "500px", out.extra='style="display: block; margin-left: auto; margin-right: auto; background-color: #000; padding:3px;"'}
knitr::include_graphics("img/unix/mac-filesystem-from-root.png") 
```
...
How do we know what is our working directory? The first command we learn is `pwd`, which stands for _print working directory_.  This command returns the working directory.
...
In Unix we use the shorthand `~` as a nickname for your home directory. So, for example, if `docs` in the path to a directory [AN: change "in" to "is"?] in your home directory,  _docs_ can be written like this `~/docs`. 
...
```{r, echo=FALSE,  out.width = "500px", out.extra='style="display: block; margin-left: auto; margin-right: auto; background-color: #000; padding:3px;"'}
knitr::include_graphics("img/unix/filesystem-vertical.png") 
```
...
So in all the `mv` examples above, you can switch `mv` to `cp` and they will copy instead of move with one exception.
...
In RStudio when you start a new project, you can pick _Exsting Directory_ instead of _New directory_:
...
In this file, we introduce a command we have not seen: `save`. The `save` command in R saves objects into what is called an _rda file_, _rda_ stands for R data. We recommend using the `.rda` suffix on files saving R objects. You will see that `.RData` is also used. 
...
[AN: check that above notes are complete. anaylysis.R seems to be incomplete]
