...
The 
We have now described several data visualization techniques and are ready to learn how to create them in R. Throughout the book, we will be using the [ggplot2](http://ggplot2.org) package. 
...
knitr::include_graphics("img/ggplot2-cheatsheeta.png")
```

```{r, echo=FALSE}
##https://ugoproto.github.io/ugo_r_doc/img/visualization_cs/ggplot2-cheatsheetb.png
knitr::include_graphics("img/ggplot2-cheatsheetb.png")
```
...
16. Now we are going to use the `geom_histogram` function to make a histogram of the heights in the `height` data frame. When reading the documentation for this function we see that it requires just one mapping, the values to be used for the histogram. Make a histogram of all the plots. 
...
17. Now create a ggplot object using the pipe to assign the heights data to a ggplot object an assign `height` to the x values through the `aes` function


18. Now we are ready to add a layer to actually make the histogram. Use the object `p` you have just created and the `geom_histogram` function to make the histogram

19. Note that when we run the code in the previous exercise we get the warning
...
20. Now instead of histogram we are going to make a smooth density plot. In this case we will not make a object. instead we will render the plot with one line of code. Change the code previous used
...
21. Now we are going to make a density plot for males and females separately. We can do this using any of the following arguments group. We do this through the aesthetic mapping as each point need to be assigned the calculation of a density.
...
22. We can also assign groups through the `fill` argument. This has the added benefit that it uses color to distinguish the groups. Like this:
