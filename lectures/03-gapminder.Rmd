---
title: "Case study: Trends in world health and economics"
...
This plot clearly shows that the majority of countries have moved from the _developing world_ cluster to the _western world_ one. In 2012, the western versus developing world view no longer makes sense. This is particularly clear when comparing Europe to Asia, the latter which includes several countries that have made great improvements. 
...
Start by creating a table with the data you need
```{r}
years <- 1960:2010
...
8. Edit the code above but now use `facet_grid` to show a different density for 1970 and 2010. Hint change the way you call `filter`to include both years and the add `facet_grid(.~year)` at the end.
